...
impl Language for Nix {
    fn name(&self) -> &'static str { "Nix" }
    fn extensions(&self) -> &'static [&'static str] { &["nix"] }
    fn grammar_name(&self) -> &'static str { "nix" }
    ...
    fn nesting_nodes(&self) -> &'static [&'static str] {
        &["attrset_expression", "let_expression", "function_expression"]
    }
    ...
    fn extract_container(&self, node: &Node, content: &str) -> Option<Symbol> {
        ...
        if !matches!(kind_str, "attrset_expression" | "let_expression" | "rec_attrset_expression") {
            return None;
        }
    ...
    fn module_name_to_paths(&self, module: &str) -> Vec<String> {
        vec![
            format!("{}.nix", module),
            format!("{}/default.nix", module),
        ]
    }

    fn lang_key(&self) -> &'static str { "nix" }
    ...
    fn find_package_cache(&self, _project_root: &Path) -> Option<PathBuf> {
        PathBuf::from("/nix/store").is_dir().then(|| PathBuf::from("/nix/store"))
    }

    fn indexable_extensions(&self) -> &'static [&'static str] { &["nix"] }
    fn package_sources(&self, _project_root: &Path) -> Vec<crate::PackageSource> { Vec::new() }

    fn should_skip_package_entry(&self, name: &str, is_dir: bool) -> bool {
        use crate::traits::{skip_dotfiles, has_extension};
        if skip_dotfiles(name) { return true; }
        if is_dir && name == "result" { return true; }
    ...
    fn package_module_name(&self, entry_name: &str) -> String {
        entry_name.strip_suffix(".nix").unwrap_or(entry_name).to_string()
    }
