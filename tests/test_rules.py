...
from moss_orchestration.rules import (
    CodeContext,
    Location,
    RuleContext,
    RuleEngine,
...
from moss_orchestration.rules.backends import get_backend, list_backends
from moss_orchestration.rules.backends.deps import (
    DepsBackend,
...
import os
from pathlib import Path
from moss_intelligence.skeleton import get_skeleton
""")
...
import os
from pathlib import Path
from moss_intelligence.skeleton import get_skeleton, SkeletonOptions
""")

        backend = DepsBackend()
        result = backend.analyze(test_file, pattern="imports_from:moss_intelligence.skeleton")
...
""")
        ...
        assert len(result.violations) == 1
        assert "moss_intelligence.skeleton" in result.violations[0].message
